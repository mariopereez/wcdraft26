...
const GRUPOS_WC2026 = {
  ...
  "B":["Canadá","Catar","Suiza","Repesca 2"],
...
function ownerTag(od) { if(!od) return ''; return `<span class="owner-pill" style="background:${getPlayerColor(od.owner)}">${od.owner}</span>`; }
function getMyTeams() {
  const s = new Set();
...
function renderResults() {
  ...
  if(banner) banner.innerHTML=isAdmin()?`<div class="notice warn" style="margin-bottom:.7rem">🔧 Admin · Edita resultados directamente aquí</div>`:`<div style="background:rgba(74,85,104,.1);border:1px solid var(--border);border-radius:8px;padding:.35rem .7rem;font-size:.7rem;color:var(--muted);font-family:'Barlow Condensed';margin-bottom:.7rem">👁️ Solo el admin puede editar · se actualiza en tiempo real</div>`;
  ...
  document.getElementById('results-grid').innerHTML=teams.map(t=>{
    ...
    const matchHtml=teamMs.length>0?`<div class="res-matches-section"><div class="res-matches-title">Partidos</div>${teamMs.map(m=>{const h=nameES(m.homeTeam?.name||''),a=nameES(m.awayTeam?.name||'');const isHome=h===t;const opp=isHome?a:h;const st=m.status;let sc='',scCls='tbd';if(st==='FINISHED'){const ms=isHome?m.score?.fullTime?.home??'-':m.score?.fullTime?.away??'-';const os=isHome?m.score?.fullTime?.away??'-':m.score?.fullTime?.home??'-';sc=`${ms}–${os}`;scCls=ms>os?'win':ms<os?'loss':'draw';}else if(st==='IN_PLAY'||st==='PAUSED'){sc='EN JUEGO';scCls='win';}else{sc=formatTime(m.utcDate);}const oo=getOwnerData(opp);return `<div class="res-match-row"><div class="res-match-opp">${flagImg(opp,'')}<span>${opp}</span>${oo?`<span class="owner-pill" style="background:${getPlayerColor(oo.owner)};margin-left:.2rem">${oo.owner}</span>`:''}</div><span class="res-match-score ${scCls}">${sc}</span><span class="res-match-meta">${formatDate(m.utcDate)}</span></div>`;}).join('')}</div>`:'';
    if(isAdmin()){return `<div class="res-card" id="rc-${key}" style="${isMyTeam?'border-color:rgba(245,197,24,.25);background:rgba(245,197,24,.025)':''}"><div class="res-card-top"><span class="res-name">${flagImg(t,'md')} ${t}${isMyTeam?'<span class="my-team-tag">TÚ</span>':''}${!isMyTeam&&ownerD?ownerTag(ownerD):''}</span><span class="res-pts" id="rpts-${key}">${Math.round((gp+ep)*10)/10} pts</span></div><div class="ved-row"><div class="ved-col ved-v"><div class="ved-lbl">🟢 V</div><div class="ved-ctrl"><button class="ved-btn" onclick="adjVED('${t}','pg',-1)">−</button><span class="ved-val" id="v-${key}-pg">${r.pg||0}</span><button class="ved-btn" onclick="adjVED('${t}','pg',1)">+</button></div></div><div class="ved-col ved-e"><div class="ved-lbl">🟡 E</div><div class="ved-ctrl"><button class="ved-btn" onclick="adjVED('${t}','pe',-1)">−</button><span class="ved-val" id="v-${key}-pe">${r.pe||0}</span><button class="ved-btn" onclick="adjVED('${t}','pe',1)">+</button></div></div><div class="ved-col ved-d"><div class="ved-lbl">🔴 D</div><div class="ved-ctrl"><button class="ved-btn" onclick="adjVED('${t}','pd',-1)">−</button><span class="ved-val" id="v-${key}-pd">${r.pd||0}</span><button class="ved-btn" onclick="adjVED('${t}','pd',1)">+</button></div></div></div><div class="toggle-row"><button class="tog ${r.r16?'on':''}" onclick="togResult('${t}','r16')">16avos</button><button class="tog ${r.r8?'on':''}" onclick="togResult('${t}','r8')">Octavos</button><button class="tog ${r.r4?'on':''}" onclick="togResult('${t}','r4')">Cuartos</button><button class="tog ${r.semi?'on':''}" onclick="togResult('${t}','semi')">Semis</button><button class="tog ${r.final?'on':''}" onclick="togResult('${t}','final')">Final</button><button class="tog ganador ${r.ganador?'on':''}" onclick="togResult('${t}','ganador')">🥇 Campeón</button><button class="tog bronce ${r.bronce?'on':''}" onclick="togResult('${t}','bronce')">🥉 Bronce</button></div>${matchHtml}</div>`;}
    else{const stages=[];if(r.r16)stages.push('16av');if(r.r8)stages.push('Oct');if(r.r4)stages.push('Cto');if(r.semi)stages.push('SF');if(r.final)stages.push('F');if(r.ganador)stages.push('🥇');if(r.bronce)stages.push('🥉');return `<div class="res-card" style="${isMyTeam?'border-color:rgba(245,197,24,.25);background:rgba(245,197,24,.025)':''}"><div class="res-card-top"><span class="res-name">${flagImg(t,'md')} ${t}${isMyTeam?'<span class="my-team-tag">TÚ</span>':''}${!isMyTeam&&ownerD?ownerTag(ownerD):''}</span><span class="res-pts">${Math.round((gp+ep)*10)/10} pts</span></div><div style="display:flex;gap:.8rem;font-family:'Barlow Condensed';font-size:.82rem;padding:.3rem 0"><span style="color:var(--cyan)">${r.pg||0}V</span><span style="color:var(--orange)">${r.pe||0}E</span><span style="color:var(--red)">${r.pd||0}D</span><span style="color:var(--muted)">${total3}/3</span></div>${stages.length?`<div style="display:flex;flex-wrap:wrap;gap:.2rem">${stages.map(s=>`<span style="font-size:.62rem;padding:.08rem .35rem;border-radius:8px;background:rgba(46,196,182,.1);color:var(--cyan);font-family:Barlow Condensed;font-weight:700">${s}</span>`).join('')}</div>`:''} ${matchHtml}</div>`;}
  }).join('');
}
function renderTodosMatches() {
  ...
  wrap.innerHTML=Object.entries(byDay).map(([day,dms])=>`<div class="match-day-group"><div class="match-day-label">${day}</div><div style="background:var(--surface);border:1px solid var(--border);border-radius:11px;overflow:hidden;margin-bottom:.4rem">${dms.map(m=>{const h=nameES(m.homeTeam?.name||'TBD'),a=nameES(m.awayTeam?.name||'TBD');const isMyH=myTeams.has(h),isMyA=myTeams.has(a);const ho=getOwnerData(h),ao=getOwnerData(a);const st=m.status;let sc='';if(st==='FINISHED')sc=`${m.score?.fullTime?.home??0}–${m.score?.fullTime?.away??0}`;else if(st==='IN_PLAY'||st==='PAUSED')sc=`<span style="color:var(--red)">${m.score?.fullTime?.home??0}–${m.score?.fullTime?.away??0}</span>`;else sc=formatTime(m.utcDate);return `<div style="display:grid;grid-template-columns:1fr 85px 1fr;align-items:center;gap:.4rem;padding:.45rem .9rem;border-bottom:1px solid rgba(42,54,80,.15);${(isMyH||isMyA)?'background:rgba(245,197,24,.04)':''}"><div style="display:flex;align-items:center;gap:.35rem;font-family:'Barlow Condensed';font-size:.82rem;font-weight:700">${flagImg(h,'md')} ${h}${ho?`<span class="owner-pill" style="background:${getPlayerColor(ho.owner)}">${ho.owner}</span>`:''}</div><div style="text-align:center;font-family:'Bebas Neue';font-size:1rem">${sc}<div style="font-size:.52rem;color:var(--muted2);font-family:'Barlow Condensed'">${stN[m.stage]||m.group||''}</div></div><div style="display:flex;align-items:center;gap:.35rem;justify-content:flex-end;font-family:'Barlow Condensed';font-size:.82rem;font-weight:700">${ao?`<span class="owner-pill" style="background:${getPlayerColor(ao.owner)}">${ao.owner}</span>`:''} ${a} ${flagImg(a,'md')}</div></div>`;}).join('')}</div></div>`).join('');
}
...
function renderClasificacion() {
  ...
  document.getElementById('clas-grid').innerHTML=ranking.map((r,i)=>{
    ...
    return `<div class="clas-card ${i===0?'rank-1':i===1?'rank-2':i===2?'rank-3':''} ${isMe?'is-me':''}" onclick="toggleClasPlayer('${r.name}')"><div class="clas-rank ${i===0?'p1':i===1?'p2':i===2?'p3':'pn'}">${medals[i]||i+1}</div><div class="clas-identity">${avatarEl(r.name,'',36)}<div><div class="clas-name">${r.name}${isMe?' ⭐':''}</div></div></div><div class="clas-flags">${teams}</div><div class="clas-pts-block"><div class="clas-pts-total">${r.total}</div><div class="clas-pts-detail">${isExpanded?'▲':'▼'}</div></div><div class="clas-bar" style="grid-column:1/-1"><div class="clas-bar-fill" style="width:${pct}%"></div></div>${isExpanded?`<div style="grid-column:1/-1">${detailHtml}</div>`:''}</div>`;
  }).join('');
}
...
const myTeamsList = draft[myDraftKey] || [];
  ...
  cont.innerHTML=`<div class="yo-hero"><div class="yo-hero-top"><div style="display:flex;flex-direction:column;align-items:center;gap:.3rem">${bigAv}<label style="cursor:pointer;font-size:.7rem;color:var(--muted);font-family:'Barlow Condensed';display:flex;align-items:center;gap:.3rem;margin-top:.4rem">📷 Cambiar foto<input type="file" accept="image/*" style="display:none" onchange="handleYoPhotoUpload(this)"></label></div><div><div class="yo-name">${myName}</div><div class="yo-rank-lbl">Posición #${myRank} de ${PARTICIPANTES.length}</div><div class="yo-total">${myScore.total} pts</div></div><button class="btn btn-outline btn-sm" onclick="showPlayerCard()" style="margin-left:auto;align-self:flex-start">🃏 Tarjeta</button></div><div class="yo-stats"><div class="yo-stat"><div class="v">${myScore.total}</div><div class="l">Puntos totales</div></div><div class="yo-stat"><div class="v">${myScore.grp}</div><div class="l">Pts grupos</div></div><div class="yo-stat"><div class="v">${myScore.elim}</div><div class="l">Pts elim ×mult</div></div></div><div style="border-top:1px solid var(--border);margin-top:1rem;padding-top:.7rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap"><span style="font-family:'Barlow Condensed';font-size:.78rem;color:var(--muted)">Predicciones acertadas:</span><span style="font-family:'Bebas Neue';font-size:1.3rem;color:var(--gold)">${calcPredPts(myName)} pts</span></div></div><div id="yo-sim-wrap"></div><div class="section-title">🎽 <span class="accent">Mis</span> Selecciones</div><div class="yo-equipos">${teamDetails.length>0?teamDetails.map(({t,i,grp,er,em,tot,st,r})=>`<div class="yo-eq" style="border-color:${TIER_DARK[i]}44"><span class="yo-mult-badge" style="background:${TIER_DARK[i]}30;color:${TIER_DARK[i]}">×${MULTS[i]||1} · R${i+1}</span><div class="yo-eq-top">${flagImg(t,'xl')}<div><div class="yo-eq-name">${t}</div><div class="yo-eq-sub">${r.pg||0}V · ${r.pe||0}E · ${r.pd||0}D</div></div></div><div class="yo-eq-pts">${tot} pts</div><div class="yo-eq-sub">Grupos: ${grp} · Elim: ${er}×${MULTS[i]||1}=${em}</div>${st.length>0?`<div class="yo-elim-tags">${st.map(s=>`<span class="yo-elim-tag${s.includes('CAMPEÓN')||s.includes('Bronce')?' gold':''}">${s}</span>`).join('')}</div>`:'<div style="font-size:.7rem;color:var(--muted2);margin-top:.4rem;font-family:Barlow Condensed">Sin progreso eliminatorio aún</div>'}</div>`).join(''):`<div style="color:var(--muted);font-family:'Barlow Condensed';padding:1rem 0">No tienes selecciones asignadas aún</div>`}</div><!-- FIX 3: Acciones de cuenta en Yo -->
<div class="section-title"><span class="accent">Mi</span> cuenta</div>
