...
function renderMatchCard(m, myTeams) {
  ...
  return `<div class="match-card ${isMyMatch?'my-team':''}"><div class="match-team">${flagImg(h,'lg')}<span class="match-name">${window.tr("country_" + h)}${isMyH?`<span class="my-badge">${myName}</span>`:''}${hot}</span></div><div class="match-vs">${scoreHtml}${statusHtml}</div><div class="match-team away">${flagImg(a,'lg')}<span class="match-name away">${window.tr("country_" + a)}${isMyA?`<span class="my-badge">${myName}</span>`:''}${aot}</span></div></div>`;
}
...
function switchResTab(tab, btn) {
  document.querySelectorAll('#page-resultados .res-tab').forEach(t=>t.classList.remove('active')); if(btn) btn.classList.add('active'); resTabActive=tab;
  const resGruposFilters = document.getElementById('res-grupos-filters'); if(resGruposFilters) resGruposFilters.style.display = tab==='grupos'?'flex':'none';
  const resultsGrid = document.getElementById('results-grid'); if(resultsGrid) resultsGrid.style.display = tab==='grupos'?'grid':'none';
  const resTodosWrap = document.getElementById('res-todos-wrap'); if(resTodosWrap) resTodosWrap.style.display = tab==='todos'?'block':'none';
...
function renderBracket() {
  ...
  bc.innerHTML=BRACKET_ROUNDS.map(({key,label,slots:slotCount})=>{
    ...
    return `<div class="bracket-round"><div class="bracket-round-title">${label}</div><div class="bracket-round-matches">${slots.map(m=>{if(!m)return `<div class="bracket-slot"><div class="bracket-match"><div class="bracket-match-team bracket-tbd"><span style="font-size:.68rem">${window.tr("bracket_tbd")}</span></div><div class="bracket-match-team bracket-tbd"><span style="font-size:.68rem">${window.tr("bracket_tbd")}</span></div></div></div>`;const h=nameES(m.homeTeam?.name||'?'),a=nameES(m.awayTeam?.name||'?');const gh=m.score?.fullTime?.home,ga=m.score?.fullTime?.away;const winner=m.status==='FINISHED'?getMatchWinnerTeamName(m):null;const ho=getOwnerData(h),ao=getOwnerData(a);return `<div class="bracket-slot"><div class="bracket-match"><div class="bracket-match-team ${winner===h?'winner':''} ${myTeams.has(h)?'my-team':''}">${flagImg(h)} ${window.tr("country_" + h)}${ho?`<span class="bracket-owner" style="${getPlayerBadgeStyle(ho.owner)}">${ho.owner}</span>`:''}${gh!=null?`<span class="bracket-score">${gh}</span>`:''}</div><div class="bracket-match-team ${winner===a?'winner':''} ${myTeams.has(a)?'my-team':''}">${flagImg(a)} ${window.tr("country_" + a)}${ao?`<span class="bracket-owner" style="${getPlayerBadgeStyle(ao.owner)}">${ao.owner}</span>`:''}${ga!=null?`<span class="bracket-score">${ga}</span>`:''}</div></div></div>`;}).join('')}</div></div>`;
  }).join('');
}
...
window.renderPorraCardHtml = function() {
  ...
  const playersHist = PARTICIPANTES.map(p => {
    ...
    const aciertos = [];
    Object.entries(pMatches).forEach(([mid, pred]) => {
      const matchObj = typeof matches !== 'undefined' ? matches.find(x => String(x.id) === String(mid)) : null;
      if (matchObj && matchObj.status === 'FINISHED' && matchObj.score?.fullTime?.home != null) {
        if (matchObj.score.fullTime.home === pred.h && matchObj.score.fullTime.away === pred.a) {
          pts += 2;
          aciertos.push({ m: matchObj, pred });
        }
      }
    });
  ...
  playersHist.forEach(({ p, pts, aciertos }) => {
    ...
    if (aciertos.length > 0) {
      aciertosHtml = `<div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap:.6rem; margin-top:.8rem">` + 
        aciertos.map(({ m: matchObj, pred }) => {
          const hName = window.tr("country_" + (typeof nameES !== 'undefined' ? nameES(matchObj.homeTeam?.name||'') : matchObj.homeTeam?.name));
          const aName = window.tr("country_" + (typeof nameES !== 'undefined' ? nameES(matchObj.awayTeam?.name||'') : matchObj.awayTeam?.name));
          const hImg = typeof flagImg !== 'undefined' ? flagImg((typeof nameES !== 'undefined' ? nameES(matchObj.homeTeam?.name||'') : matchObj.homeTeam?.name), 'md') : '';
          const aImg = typeof flagImg !== 'undefined' ? flagImg((typeof nameES !== 'undefined' ? nameES(matchObj.awayTeam?.name||'') : matchObj.awayTeam?.name), 'md') : '';
